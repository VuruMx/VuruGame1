...
function process_point($key, $remove = false, $userid = null) {
    $point = config($key, 10);
    $newPoint = (!$remove) ?  get_user()['point'] + $point : get_user()['point'] - $point;
    $userid = ($userid) ? $userid : get_userid();
    db()->query("UPDATE members SET point =? WHERE id=?", $newPoint,get_userid());
    $query = db()->query("SELECT badge_id FROM user_badges WHERE user_id=? ", $userid);
    $ids = array();
    while($fetch = $query->fetch(PDO::FETCH_ASSOC)) {
        $ids[] = $fetch['badge_id'];
    }
